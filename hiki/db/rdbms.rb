...
require 'rubygems'
require 'sequel'
require 'sequel/plugins/force_encoding'

module Hiki
  class HikiDB_rdbms < HikiDBBase
    # Monkey patch for MySQL to use #force_hash_encoding
    extend Sequel::Plugins::ForceEncoding::ClassMethods
    include Sequel::Plugins::ForceEncoding::InstanceMethods
    def model; self.class; end
    def force_array_encoding(array)
      fe = model.forced_encoding
      array.map {|v| v.respond_to?(:force_encoding) ? v.force_encoding(fe) : v} if fe
    end
    ...
    def open_db
      Sequel::Plugins::ForceEncoding.configure self.class, @conf.charset # Monkey patch
    ...
    # TODO: Implement appropriate algorithm for RDBMS
    def search(w)
      super
    end
    ...
    def info(p)
      ...
      force_hash_encoding(page) # Monkey path for MySQL
    end
    ...
    def get_attribute(p, attribute)
      case attribute
      when :keyword
        force_array_encoding(@db[:keyword].filter(:page_name => p).select_map(:keyword)) # Monkey patch for MySQL
      when :references
        get_references(p)
      else
        record = @db[:page].filter(:name => p).select(attribute).first
        force_hash_encoding(record) unless record.nil? # Monkey patch for MySQL
    ...
    def get_references(p)
      force_array_encoding(@db[:reference].filter(:to => p).select_map(:from)) # Monkey path fo MySQL
    end
    ...
    # It should be used out of it if needed
